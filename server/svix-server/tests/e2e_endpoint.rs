...
async fn test_endpoint_rotate_signing_e2e() {
    ...
    let secret3_key = EndpointSecret::generate_symmetric().unwrap();
...
async fn test_endpoint_rotate_signing_symmetric_and_asymmetric() {
    ...
    let secret_1 = EndpointSecret::generate_symmetric().unwrap();
    // Asymmetric key
                ...
                let found =
                    signatures
                        ...
                        .any(|signature| {
                            ...
                            .unwrap();
                            key.0.pk.verify(to_sign, &sig).is_ok()
                        });
                assert!(found);
            }
        }
...
async fn test_legacy_endpoint_secret() {
    ...
    let secret_throwaway = EndpointSecret::generate_symmetric().unwrap();
