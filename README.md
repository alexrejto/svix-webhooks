...
Svix uses `ed25519(sha512(m))` for signing the webhook messages. It uses a very similar structure to the one used by symmetric keys, the main difference is that the whole thing is hashed with `sha512` before being signed.
